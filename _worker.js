  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/scan' && request.method === 'POST') {
      try {
        ...
        // Lista ordenada de modelos a probar automáticamente
        ...
        // Iterar hasta encontrar un modelo aceptado por la API Key
        for (const model of candidateModels) {
          ...
          // Si el modelo funcionó correctamente, responder al cliente
        ...
        // Si ninguno de los modelos funcionó, devolver la respuesta explícita de Google
        return new Response(JSON.stringify({ 
          error: 'Ningún modelo candidate respondió con éxito', 
          detalle_google: lastApiError 
        }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
