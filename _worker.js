...
<head>
  ...
  <style>
    * { box-sizing: border-box; font-family: system-ui, -apple-system, sans-serif; }
    ...
    #debugLog { background: #000; color: #00ff66; font-family: monospace; padding: 12px; border-radius: 6px; height: 120px; overflow-y: auto; font-size: 12px; margin-top: 10px; border: 1px solid #333; white-space: pre-wrap; }
...
<script>
  ...
  function fileToBase64(file) {
    return new Promise(function(resolve, reject) {
      ...
      reader.onload = function() {
        ...
        if (typeof res === 'string' && res.includes(',')) resolve(res.split(',')[1]);
        else reject(new Error("Error leyendo archivo Base64"));
      };
  ...
  async function guardarComprobante() {
    try {
      ...
      try {
        var response = await fetch('/api/guardar', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
        if (response.ok) log("Sincronizado con servidor.");
      } catch (e) {
        log("Guardado localmente (servidor offline).");
      }
